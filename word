Sub UbahKeTerbilang()
    ...
    ' Mengambil teks yang sedang diblok/dipilih di Word
    angka = Selection.Text
    
    ' Bersihkan karakter non-angka (seperti titik atau koma)
    angka = Replace(Replace(angka, ".", ""), ",", "")
    
    ' Memanggil fungsi Terbilang (Pastikan fungsi TERBILANG ada di modul yang sama)
    If IsNumeric(angka) Then
        hasil = TERBILANG(CDbl(angka))
        Selection.TypeText Text:=hasil
    Else
        MsgBox "Mohon blok angka terlebih dahulu!"
    End If
End Sub
